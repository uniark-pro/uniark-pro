...
# 懒加载：双 market 重构后，import data 会传染地 import 本模块。如果
# 在这里 module-load 时就实例化 Client()，会触发对 binance.com 的
# 网络 ping —— 哪怕用户当前只想用美股 / 当前网络封锁了 binance，仍会
# 在启动阶段抛 BinanceAPIException。改成懒加载后，只有真正请求 crypto
# 数据时才连 binance。
_client = None
...
# Client.KLINE_INTERVAL_* 是类属性，访问它们不触发网络。
INTERVAL_MAP = {
