...
# ─────────────────────────────────────────────────────────────────────────────
# 极值补检（find_missed_extremes）的过短反向段过滤阈值
# ─────────────────────────────────────────────────────────────────────────────
# find_missed_extremes 的几何判据极宽松——只要相邻两段 (S1, S2) 中后一段的
# 同类价格极值 越过/触及 前一段，就算"漏掉的极值"。这个判据在趋势中段的
# 长反向段对上非常稳健，但在以下场景会刷出噪声：
#
#   持续下跌中，价格一路创新低的同时 hist 频繁翻号，切出一系列短小的
#   红 / 绿段（每段 1-3 根 K 线）。每对相邻短段几乎必然满足"后段低点 ≤
#   前段低点"（因为整体趋势就是创新低），于是每一对都触发一条"底背离·极值"，
#   钻取卡片刷屏，且没有一条对应真正的趋势转折。顶部对称同理。
#
# 物理上这种短段是 MACD hist 在过零线附近的微小抖动，不构成两个独立的
# 动量阶段——把它们当作"相邻段"做价格越界比较，几何上成立但语义上无效。
#
# 约束：S1 段和 S2 段的原始 K 线根数都必须 ≥ MISSED_EXTREME_MIN_BARS，
# 才允许进入价格比较；任一过短则跳过该对。
#
# 阈值 4 的来历：经验值。多数主流周期（周/日/小时）下，一段动量阶段
# 至少需要 4 根 K 线才能形成有意义的能量积累；不足 4 根的段通常是
# 横盘抖动或单根反向 K 引发的瞬时翻号。如需调整改这一处即可。
#
...
def find_missed_extremes(hist_series, low_series, high_series):
    ...
    过短反向段过滤（MISSED_EXTREME_MIN_BARS）
    ----------------------------------------
    "相邻段比较"的几何判据在趋势中段的长反向段对上稳健，但在持续单边
    走势里 hist 频繁过零会切出一串短段（每段 1-3 根 K 线），每对都几乎
    必然满足价格越界条件（因为趋势本身在创新极值），形成噪声刷屏。

    例：BTC 周线 2018 下跌段，价格一路新低，hist 在零轴附近频繁翻号，
    切出多个短段；按裸判据会在 2018-09 顶部附近刷出"顶背离·极值"、
    在 2018-11 下跌中刷出"底背离·极值"，两条都不对应真正的趋势转折，
    ...
    for idx in range(1, len(raw)):
        ...
        # 过短反向段过滤：S1 / S2 任一不足 MISSED_EXTREME_MIN_BARS 根，
        # 视为零轴附近的瞬时翻号噪声，不参与价格越界判定。
        if (prev['bars'] < MISSED_EXTREME_MIN_BARS
                or seg['bars'] < MISSED_EXTREME_MIN_BARS):
            continue
