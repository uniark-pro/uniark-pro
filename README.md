...
工具的多尺度结构观察,可以通过两个真实案例来理解。研究材料用 MU(美光)
和 BTC(比特币)的真实历史 K 线,展示"背离钻取"功能如何把不同时间尺度
上的 MACD 背离结构关系可视化。

📖 [**MACD 背离在不同时间尺度上的表现:MU 与 BTC 两个案例研究**](docs/tutorial/macd_divergence_study.md)
...
├── docs/
│   ├── tutorial/                  ← MU 与 BTC 案例研究
│   └── personal_reflection.md     ← 写给朋友们的心得文章
├── images/                        ← 示例图
