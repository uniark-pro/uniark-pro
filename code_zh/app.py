...
K 线分析 - Web App（含设置面板）
=================================
入口：选币种 → 选入口周期（weekly/3day/daily）→ 选时段 → 看图
钻取：根据当前图的 K 线根数动态计算段数 = floor(N_next/385)+1
后端 /generate 返回图、实际窗口、根数。

入口周期：
  ENTRY_INTERVALS（来自 settings 模块）定义主界面允许的入口周期。
  每个入口对应一份独立的时段列表（settings.ranges[interval]）。

设置：
  顶栏 ⚙ 按钮打开设置视图，可改语言、币种、各入口周期的时段列表。
  改动通过 /settings POST 写入 user_settings.json，与 main.py 共享。
"""
import os
import sys
import io
...
<script>
  ...
  let selInterval = ENTRY_INTERVALS[0];   // 默认第一个 = weekly
  // 每个入口周期记一个被选中的时段下标
  let selRngIdxByIv = {};
  ...
  let stack = [];          // 导航栈
  let prevView = 'view-select';   // 设置返回时去哪

  // 设置面板的临时草稿（取消时丢弃）
  let draftLang = curLang;
  let draftSyms = [];
  let draftRngs = {};       // {iv: [...]}
  let draftRngIv = ENTRY_INTERVALS[0];   // 当前编辑哪个 tab
  ...
  function buildIntervalPicker() {
    ...
    ENTRY_INTERVALS.forEach(iv => {
      ...
      el.onclick = () => {
        selInterval = iv;
        buildIntervalPicker();
        buildRangePicker();        // 切了周期 → 时段列表跟着重建
      };
      grid.appendChild(el);
    });
  }
