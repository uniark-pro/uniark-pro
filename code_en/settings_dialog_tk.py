"""
Desktop settings dialog (Tkinter Toplevel)
==========================================
Split into its own file to keep main.py thin.

Single public export:
    open_settings_dialog(parent, lang, on_save_callback)

The dialog has three sections: language / symbols / time ranges.
The time-range section is split into tabs by entry interval
(settings.ENTRY_INTERVALS); each tab has its own independent
"list + add form".

On save: validate, write to settings.SETTINGS_FILE, then call
...
# ── Color palette (same as main.py; sync both files when changing) ──
BG        = "#1e1e2e"
BG_CARD   = "#2a2a3e"
ACCENT    = "#7c6af7"
FG        = "#e0e0f0"
...
# ── Entry placeholder (Tk Entry has no native placeholder; emulate) ──
def _attach_placeholder(entry, text):
    """Emulate a placeholder on a Tk Entry: show grey hint when
    unfocused and empty; clear on focus or input."""
    def show_ph():
        entry.delete(0, 'end')
        entry.insert(0, text)
...
def _entry_value(entry):
    """Return the real value of an Entry; treat placeholder state as empty."""
    if getattr(entry, '_is_placeholder', False):
        return ''
...
def _clear_entry(entry, ph_text):
    """Clear an Entry and re-show its placeholder."""
    entry.delete(0, 'end')
    entry.configure(fg=FG_DIM)
...
def open_settings_dialog(parent, lang, on_save_callback):
    ...
    lang : str
        Current language ('en' | 'zh') — determines initial dialog text.
    on_save_callback : callable(new_settings: dict)
        Called on successful save with the saved settings dict.
    """
    cur = settings.load_settings()
    syms = list(cur['symbols'])
    # rngs_by_iv: one independent list per entry interval
    ...
    # Which entry interval's ranges are currently being edited
    # (the active tab).
    ...
    # ── (3) Time ranges (tabs by entry interval) ────────────────────
    ...
    def rebuild_iv_tabs():
        """Rebuild tab text on language change."""
    ...
    def do_save():
        # Validation: every entry interval must have at least one range.
        for iv in ENTRY_INTERVALS:
            if len(rngs_by_iv[iv]) == 0:
                rng_iv_var.set(iv)        # Jump to that tab so the user sees the issue
                update_iv_tab_highlight()
                render_rngs()
    ...
    # ── Close: unbind global mousewheel handlers ────────────────────
    def _close():
        try:
    ...
    # ── Apply text (also called when language changes) ──────────────
