...
支持的粒度
----------
支持 weekly、3day、daily、1h 四个粒度——这与 settings 的 stock entry
intervals（weekly/3day/daily）+ navigation 末端钻取 1h 一致。

  - weekly / daily / 1h：yfinance 原生粒度，直接拉取
  - 3day：yfinance 无原生 3 日粒度。本 adapter 按 daily 拉取后用
    resample('3D', origin='epoch') 重采样合成——3 自然日一根 K 线，
    bin 边界锚定到 1970-01-01，全局固定、可复现，不随 lookback /
    拉取窗口漂移（跟 crypto binance 原生 3day 的固定日历对齐一致）。
  - 4h / 30m / 15m / 5m 暂不开放，请求时抛 NotImplementedError：
    intraday 短粒度受 yfinance 60 天窗口限制 + 美股盘前盘后断点 gap，
    钻取语义需单独设计，留作后续扩展。
...
# yfinance 无原生 3 日粒度。3day 按 daily 拉取后由 _resample_to_3day
# 重采样合成。其他派生粒度（4h 等）留待后续，暂不开放。
DERIVED_FROM_DAILY = ('3day',)
...
def _resample_to_3day(df):
    ...
    全局固定、可复现——不随 lookback 大小或拉取窗口起点漂移。这点很关键：
    plot_kline 会先往前多拉 LOOKBACK_BARS 根再切片，若 bin 边界随拉取窗口
    漂移，同一段行情在不同 lookback 下会画出不同的 K 线。固定日历对齐也跟
    crypto 的 binance 原生 3day 行为一致。

    不直接用 df.resample('3D')：pandas 对非 tick-like 频率会忽略 origin 参数
    （会打 RuntimeWarning），bin 改为锚定到数据首行、落点随拉取窗口变化。
    手动按纪元算 bin 序号可彻底规避这个版本相关的坑。
    ...
    # bin 序号 → bin 起始日期（该 3 日窗口的第一个自然日）
...
def get_klines(symbol, interval, start_str, end_str=None):
    ...
    interval : str
        'daily' | 'weekly' | '3day' | '1h'。其他值抛 NotImplementedError。
        - daily / weekly / 3day：无历史长度限制
        - 3day：按 daily 拉取后重采样合成（yfinance 无原生 3 日粒度）
    ...
    """
    if interval not in INTERVAL_MAP and interval not in DERIVED_FROM_DAILY:
        supported = list(INTERVAL_MAP.keys()) + list(DERIVED_FROM_DAILY)
        raise NotImplementedError(
            f"美股 adapter 当前仅支持 {supported}，"
            f"收到 {interval!r}。4h/30m/15m 留待后续扩展。"
        )

    # 派生粒度（3day）没有 yfinance 原生 interval，先按 daily 拉，
    # 函数末尾再 _resample_to_3day 合成。
    yf_interval = '1d' if interval in DERIVED_FROM_DAILY else INTERVAL_MAP[interval]
